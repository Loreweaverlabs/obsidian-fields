...
- **D-036 — Pixel portraits.** Five hand-authored 64x64 pixel-art busts generated by
  `scripts/portraits.ts` (zero-dep PNG encoder; regenerate with `npm run portraits`) into
  `public/portraits/`. `cast_public.json` gained `portraitImage`; the UI falls back to the old
  initials token when absent, so recasting with real Forgotten Runes art remains a data edit.
